const {
    countUsersService, calculateRevenueService, countProductSalesService, countAvailableProductsService,
    countOrdersService, countLaptopOfBrandSold
...
class StatisticController {
    ...
    async countQuantityOfBrandSold(req, res) {
        try {
            const data = await countLaptopOfBrandSold()
            if (!data) {
                res.status(404).send('Quantity of Brand Not Found')
            }
            res.status(200).json({
                data: data,
            })
